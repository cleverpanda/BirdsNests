	...
	public void onDrops(BlockEvent.HarvestDropsEvent event) {
		if(BirdsNests.allowDecayDrops){
			...
			if ( theblock == Blocks.LEAVES || theblock == Blocks.LEAVES2||OreDictionary.getOres("treeLeaves").contains(new ItemStack(theblock)) || theblock instanceof BlockLeaves)
			{
				...
				if(event.getState().getProperties().containsKey(BirdsNests.DECAYABLE) && event.getState().getValue(BirdsNests.DECAYABLE)){
					
					int adjustedRarity = (int)Math.ceil(BirdsNests.nestRarity*BirdsNests.decayDropModifier);
					if (event.getWorld().rand.nextInt( adjustedRarity ) == 0)
					{	 
